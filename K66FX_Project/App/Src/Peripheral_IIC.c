...
#define SDA             gpio_get (Peripheral_SDA)
#define SDA0()          gpio_set (Peripheral_SDA, 0)		//IO口输出低电平
#define SDA1()          gpio_set (Peripheral_SDA, 1)		//IO口输出高电平  
#define SCL0()          gpio_set (Peripheral_SCL, 0)		//IO口输出低电平
#define SCL1()          gpio_set (Peripheral_SCL, 1)		//IO口输出高电平
#define DIR_OUT()       gpio_ddr (Peripheral_SDA, GPO)    //输出方向
#define DIR_IN()        gpio_ddr (Peripheral_SDA, GPI)    //输入方向
...
void IIC_init(void)
{
	gpio_init (Peripheral_SCL, GPO,1);
	gpio_init (Peripheral_SDA, GPO,1);
	
	
	port_init_NoAlt (Peripheral_SCL, ODO | PULLUP);//ODO
	port_init_NoAlt (Peripheral_SDA, ODO | PULLUP);
}
